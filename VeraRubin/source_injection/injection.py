...
def create_crowded_injection_catalog(ra_list, dec_list, stamp_paths, mags, min_sep=0.0005, separation_spherical=True):
    """
...
def rotate_stamp(stamp_file, rotation_angle, output_path, keep_size=False, interp_order=3, update_wcs=True):
    ...
    --------
    str or None
        Path to the rotated FITS file or None if an error occurred.
    """
    try:
        ...
        # Rotate the image
        rotated_data = ndimage.rotate(data, rotation_angle, reshape=not keep_size, order=interp_order)

        # Optionally update WCS
        if update_wcs and 'CTYPE1' in hdr:
            ...
            ### Rotate linear transformation (CD or PC)
        ...
        hdu = fits.PrimaryHDU(data=rotated_data, header=fits.Header(hdr))  # Dumps the modified WCS back to the header. Thus the FITS you are going to write will already have the rotated WCS.
        ...
        print(f"Error rotating stamp: {e}")
        return None